...
func SetupPublicRoutes(app *fiber.App, authController *public.AuthController) {
	...
	app.Use(security.DetectClientIP(authController.RedisCache))
	app.Use(security.RateLimitPerGuest(authController.RedisCache, 20, 1*time.Minute))
	...
	guest.Use(security.DetectClientIP(authController.RedisCache))
	guest.Use(security.RateLimitPerGuest(authController.RedisCache, 20, 1*time.Minute))
